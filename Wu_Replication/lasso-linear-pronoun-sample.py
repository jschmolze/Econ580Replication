...
# Add allow_pickle=True to the load function
word_counts=np.load(dir_data+"X_word_count.npz", encoding='latin1', allow_pickle=True)
# Use .item() instead of [()] to extract the sparse matrix object
...
model=LassoCV(cv=5, n_jobs = -1).fit(X_train,y_train)
...
ypred_test0 = model.predict(X_test0)
ypred_test1 = model.predict(X_test1)

np.savetxt(dir_lasso+"linear_ypred_train.txt",ypred_train)
np.savetxt(dir_lasso+"linear_ypred_test0.txt",ypred_test0)
np.savetxt(dir_lasso+"linear_ypred_test1.txt",ypred_test1)
